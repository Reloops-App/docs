...
| Audio | `.mp3`, `.m4a`, `.mp4`, `.wav`, `.flac`, `.aac`, `.ogg` | `audio/mpeg`, `audio/mp4`, `audio/wav`, `audio/x-wav`, `audio/flac`, `audio/aac`, `audio/ogg` | Audio playback, timecoded comments, embedded cover artwork when available, sharing, and download |
...
| Preview in review page | Yes | Yes | Yes | Yes | Limited |
| Comments | Yes | Yes | Yes | Yes | Asset-level only |
| Drawing or annotation tools | Yes | No | No | No | No |
| Timecoded comments | No | Yes | No | Yes | No |
...
- Images, videos, PDFs, and audio files with embedded artwork can receive thumbnails.
- MP3 ID3 artwork, FLAC picture blocks, and M4A attached artwork can be used as audio cover images when present.
- Audio files without embedded artwork show a music icon fallback in asset cards and the audio review player.
...
- records the upload `contentType` as the asset `mime_type`, with extension-based fallback for common media types when the client sends a generic binary type,
- can request thumbnail generation after upload, including embedded audio cover artwork for non-web uploads,
