...
- **Assign** the asset to an AI agent when an automation should pick up the work.
...
- **Compare versions** when the asset has more than one revision.
...
Statuses appear on each asset card in the Assets view and determine which column an asset occupies in the Kanban view.
...
The Assets view displays assets as cards arranged in a grid. Use it to browse your uploads, search for a specific file, filter by status or assignee, and take quick actions without opening each asset individually.

Hover an asset card to reveal its three-dot menu. Multi-version cards include **Compare** in that menu; single-version cards do not.

<Frame caption="Image assets in the Assets grid.">
  ![Assets view showing image asset cards in a grid](/images/quick-view-images-grid.png)
</Frame>

<Frame caption="Video assets in the Assets grid.">
...
<Frame caption="Filtering the grid down to assets currently in review.">
  ![Assets view filtered to In review status](/images/quick-view-status-in-review-grid.png)
</Frame>
...
- **Share version-locked links** with people outside your workspace.
