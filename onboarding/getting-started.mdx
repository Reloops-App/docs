...
<Steps>
  ...
  <Step title="Create your first project">
    ...
    Start with an empty project unless a template already matches your workflow — the Social Media template, for example, pre-populates folders and statuses.
  </Step>

  <Step title="Upload your first asset">
    With the project open, select the **Upload** button and choose files from your device — or drag and drop them directly onto the project assets view. Reloops supports video, images, PDFs, and other common file types. After the upload finishes, each file appears as its own asset card in the Files view, ready to open for review.
  </Step>
  ...
  <Step title="Review and collaborate">
    ...
    - **Update the status** — move the asset to **Needs Review**, **In Review**, or another status.
    - **Share with a reviewer** — generate a share link to send to a client or external stakeholder who does not have a Reloops account.
  </Step>

  <Step title="Track progress">
    As the team works through feedback, keep statuses up to date — **Needs Review**, **In Review**, **Approved**, or any custom status your workspace uses. Switch to the **Kanban** view inside the project to see all assets grouped by status in a single board. Move cards between columns to update status, and use the board as your source of truth for where each deliverable stands.
  </Step>
...
<CardGroup cols={2}>
  ...
  <Card
    title="Project Assets View"
    icon="folder-open"
    href="/projects/project-assets-view"
  >
